...
// --- 2. Préparation des dossiers / fichiers JSON ---
...
[
  usersFile, musicsFile, resetFile, creatorsFile
].forEach(file => {
...
// --- PARTIE 1 : Authentification & sessions ---
...
// --- PARTIE 3 : Download, suppression, mot de passe ---
...
app.post('/api/delete', requireLogin, (req, res) => {
  ...
  const idx        = musics.findIndex(m =>
    m.path === p && m.uploader_email === user.email
  );
...
// --- PARTIE 4 : Réinitialisation & favoris & profils ---
...
// 11. Change password from profile
...
// 12–20. Favoris, top, session, profils…
...
app.get("/api/favorites", requireLogin, (req, res) => {
  ...
  res.json({ status: "success", favorites: musics.filter(m => favs.includes(m.path)) });
});
...
app.get("/api/get-session", (req, res) => {
  if (!req.session.user) return res.status(401).json({ status: "error", message: "Non connecté." });
...
app.post("/api/update-listen", requireLogin, (req, res) => {
  ...
  const idx  = musics.findIndex(m => m.path === path);
...
// 22. Route liste créateurs externe
