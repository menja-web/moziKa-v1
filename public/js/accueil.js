...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // 🗑 Suppression / Retrait favori (confirmation)
  document.addEventListener("click", async e => {
    // ouverture de la boîte de confirmation
    ...
    if (e.target.classList.contains("addFavBtn")) {
      ...
      try {
        ...
        if (data.status === "success") {
          ...
          alert("Erreur ajout favori : " + data.message);
        }
      } catch {
...
async function loadUserFavorites() {
  ...
  if (!data.favorites || data.favorites.length === 0) {
    list.innerHTML = `<li style="text-align:center;color:#999;">
